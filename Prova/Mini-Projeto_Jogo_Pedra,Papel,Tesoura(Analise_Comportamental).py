...
# Variável global para análise
estatisticas_jogador = {'pedra': 0, 'papel': 0, 'tesoura': 0}

def obter_jogada_computador() -> str:
    # (passo 1) Define as jogadas válidas
    jogadas_validas = ['pedra', 'papel', 'tesoura']
    # (passo 2) Estratégia adaptativa: tenta jogar contra a jogada mais frequente do jogador
    if sum(estatisticas_jogador.values()) == 0:
        jogada = random.choice(jogadas_validas)
    else:
        jogada_mais_frequente = max(estatisticas_jogador, key=estatisticas_jogador.get)
        # Computador joga a jogada que vence a mais frequente do jogador
        for jogada in jogadas_validas:
            if obter_regras()[jogada] == jogada_mais_frequente:
                return jogada
        jogada = random.choice(jogadas_validas)
...
def ler_jogada() -> str:
    ...
    if jogada in estatisticas_jogador:
        estatisticas_jogador[jogada] += 1
    return jogada
...
def exibir_relatorio_final() -> None:
    print("\n=== Relatório de Estilo de Jogo ===")
    total = sum(estatisticas_jogador.values())
    if total == 0:
        print("Nenhuma jogada registrada.")
        return
    for jogada, qtd in estatisticas_jogador.items():
        porcentagem = (qtd / total) * 100
        barras = int(porcentagem // 5)
        print(f"{jogada.capitalize():7}: {qtd} vezes ({porcentagem:.1f}%) | {'█' * barras}")
    jogada_favorita = max(estatisticas_jogador, key=estatisticas_jogador.get)
    print(f"\nSua jogada mais usada foi: {jogada_favorita.upper()}")
...
def jogar() -> None:
    ...
    while True:
        ...
        print("\n--- Jogo Pedra, Papel, Tesoura com Análise ---")
